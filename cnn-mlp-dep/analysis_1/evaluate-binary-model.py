'''
    Analysis script V: 
        I: Evaluate model predictions for testing dataset and write data out to local machine for summary
'''
...
json_file = open('analysis-1.json', 'r')
loaded_model_json = json_file.read()
json_file.close()
loaded_model = model_from_json(loaded_model_json)
loaded_model.load_weights("analysis-1.h5")
...
''' split data into 4 sub frames for prediction '''
...
for df in test_split: 
    ...
    vector_data = df.drop(['file_name_x', 'label', 'path'], axis=1).values
    ...
    test_input = [ image_data, vector_data ]
    test_labels = labels
    print('generating predictions for testing subset')
    df['pred_raw'] = loaded_model.predict(test_input)
    df['prob'] = df.apply(lambda row: row['pred_raw'] * 100, axis=1)
    df['pred_label'] = df.apply(lambda row: assign_lab(row), axis=1)
...
test_eval.to_csv('./testing-data-preds-analysis-1.csv', index=False)

''' scp testing data from remote ec2 to local machine '''

scp -i plankton-net.pem ec2-user@ec2-18-191-220-118.us-east-2.compute.amazonaws.com:~/plankton/testing-data-preds-analysis-1.csv .

''' scp model checkpoints '''

scp -i plankton-net.pem ec2-user@ec2-18-191-220-118.us-east-2.compute.amazonaws.com:~/plankton/best_model_analysis_1.h5 .
scp -i plankton-net.pem ec2-user@ec2-18-191-220-118.us-east-2.compute.amazonaws.com:~/plankton/analysis-1.json .
scp -i plankton-net.pem ec2-user@ec2-18-191-220-118.us-east-2.compute.amazonaws.com:~/plankton/analysis-1.h5 .
